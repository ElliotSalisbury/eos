...
inline cv::Mat fit_shape(cv::Mat affine_camera_matrix, morphablemodel::MorphableModel morphable_model, std::vector<morphablemodel::Blendshape> blendshapes, std::vector<cv::Vec2f> image_points, std::vector<int> vertex_indices, float lambda, boost::optional<int> num_coefficients_to_fit, std::vector<float>& pca_shape_coefficients, std::vector<float>& blendshape_coefficients)
{
	...
	do // run at least once:
	{
		...
		current_pca_coeffs = fitting::fit_shape_to_landmarks_linear(morphable_model, vector<Mat>({ affine_camera_matrix }), vector<vector<Vec2f>>({ image_points }), vector<vector<int>>({ vertex_indices }), vector<Mat>({ mean_plus_blendshapes }), lambda, num_coefficients_to_fit);
 ...
 */
inline std::pair<std::vector<core::Mesh>, std::vector<fitting::RenderingParameters>> fit_shape_and_pose(const morphablemodel::MorphableModel& morphable_model, const std::vector<morphablemodel::Blendshape>& blendshapes, const std::vector<core::LandmarkCollection<cv::Vec2f>>& landmarkss, const core::LandmarkMapper& landmark_mapper, std::vector<int> image_widths, std::vector<int> image_heights, const morphablemodel::EdgeTopology& edge_topology, const fitting::ContourLandmarks& contour_landmarks, const fitting::ModelContour& model_contour, int num_iterations, boost::optional<int> num_shape_coefficients_to_fit, float lambda, boost::optional<fitting::RenderingParameters> initial_rendering_params, std::vector<float>& pca_shape_coefficients, std::vector<std::vector<float>>& blendshape_coefficientss, std::vector<cv::Vec2f>& fitted_image_points)
{
	assert(blendshapes.size() > 0);
	assert(landmarkss.size() > 0 && landmarkss.size() == image_widths.size() && image_widths.size() == image_heights.size());
	assert(landmarkss[0].size() >= 4);
	assert(image_widths[0] > 0 && image_heights[0] > 0);
	...
	if (blendshape_coefficientss.size() == 0)
	{
		for (int j = 0; j < num_images; ++j) {
			std::vector<float> blendshape_coefficients;
			blendshape_coefficients.resize(blendshapes.size());
			blendshape_coefficientss.push_back(blendshape_coefficients);
		}
	}
	...
	for (int j = 0; j < num_images; ++j) {
		Mat current_combined_shape = current_pca_shape + blendshapes_as_basis * Mat(blendshape_coefficientss[j]);
	...
	vector<vector<Vec4f>> model_pointss; // the points in the 3D shape model
	vector<vector<int>> vertex_indicess; // their vertex indices
	vector<vector<Vec2f>> image_pointss; // the corresponding 2D landmark points
	...
	for (int j = 0; j < num_images; ++j) {
		...
		for (int i = 0; i < landmarkss[j].size(); ++i) {
			auto converted_name = landmark_mapper.convert(landmarkss[j][i].name);
			if (!converted_name) { // no mapping defined for the current landmark
				continue;
			}
			int vertex_idx = std::stoi(converted_name.get());
			...
			model_points.emplace_back(vertex);
			vertex_indices.emplace_back(vertex_idx);
			image_points.emplace_back(landmarkss[j][i].coordinates);
		}

		model_pointss.push_back(model_points);
		vertex_indicess.push_back(vertex_indices);
		image_pointss.push_back(image_points);
	}
	...
	vector<vector<int>> fixed_vertex_indicess;
	vector<vector<Vec2f>> fixed_image_pointss;
	vector<fitting::RenderingParameters> rendering_paramss;
	for (int j = 0; j < num_images; ++j) {
		fitting::ScaledOrthoProjectionParameters current_pose = fitting::estimate_orthographic_projection_linear(image_pointss[j], model_pointss[j], true, image_heights[j]);
		fitting::RenderingParameters rendering_params(current_pose, image_widths[j], image_heights[j]);
		rendering_paramss.push_back(rendering_params);

		Mat affine_from_ortho = fitting::get_3x4_affine_camera_matrix(rendering_params, image_widths[j], image_heights[j]);
		blendshape_coefficientss[j] = fitting::fit_blendshapes_to_landmarks_nnls(blendshapes, current_pca_shape, affine_from_ortho, image_pointss[j], vertex_indicess[j]);
		...
		current_combined_shapes[j] = current_pca_shape + morphablemodel::to_matrix(blendshapes) * Mat(blendshape_coefficientss[j]);
		...
		fixed_vertex_indicess.push_back(vector<int>(vertex_indicess[j]));
		fixed_image_pointss.push_back(vector<Vec2f>(image_pointss[j]));
	}
	...
	for (int i = 0; i < num_iterations; ++i)
	{
		...
		for (int j = 0; j < num_images; ++j) {
			vertex_indices_w_contour.push_back(vector<int>(fixed_vertex_indicess[j]));
			image_points_w_contour.push_back(vector<Vec2f>(fixed_image_pointss[j]));
			...
			auto yaw_angle = glm::degrees(glm::eulerAngles(rendering_paramss[j].get_rotation())[1]);
			// For each 2D contour landmark, get the corresponding 3D vertex point and vertex id:
			std::tie(image_points_contour, std::ignore, vertex_indices_contour) = fitting::get_contour_correspondences(landmarkss[j], contour_landmarks, model_contour, yaw_angle, current_meshs[j], rendering_paramss[j].get_modelview(), rendering_paramss[j].get_projection(), fitting::get_opencv_viewport(image_widths[j], image_heights[j]));
			// Add the contour correspondences to the set of landmarks that we use for the fitting:
			vertex_indices_w_contour[j] = fitting::concat(vertex_indices_w_contour[j], vertex_indices_contour);
			image_points_w_contour[j] = fitting::concat(image_points_w_contour[j], image_points_contour);
			...
			{ // the left contour is the occluding one we want to use ("away-facing")
				auto contour_landmarks_ = core::filter(landmarkss[j], contour_landmarks.left_contour); // Can do this outside of the loop
				...
				auto contour_landmarks_ = core::filter(landmarkss[j], contour_landmarks.right_contour);
				std::for_each(begin(contour_landmarks_), end(contour_landmarks_), [&occluding_contour_landmarks](auto&& lm) { occluding_contour_landmarks.push_back({ lm.coordinates[0], lm.coordinates[1] }); });
			}
			auto edge_correspondences = fitting::find_occluding_edge_correspondences(current_meshs[j], edge_topology, rendering_paramss[j], occluding_contour_landmarks, 180.0f);
			image_points_w_contour[j] = fitting::concat(image_points_w_contour[j], edge_correspondences.first);
			vertex_indices_w_contour[j] = fitting::concat(vertex_indices_w_contour[j], edge_correspondences.second);

			// Get the model points of the current mesh, for all correspondences that we've got:
			model_pointss[j].clear();
			for (const auto& v : vertex_indices_w_contour[j])
			{
				model_pointss[j].push_back({ current_meshs[j].vertices[v][0], current_meshs[j].vertices[v][1], current_meshs[j].vertices[v][2], current_meshs[j].vertices[v][3] });
			}
			...
			Mat mean_plus_blendshapes = morphable_model.get_shape_model().get_mean() + blendshapes_as_basis * Mat(blendshape_coefficientss[j]);
			mean_plus_blendshapess.push_back(mean_plus_blendshapes);
		}
		pca_shape_coefficients = fitting::fit_shape_to_landmarks_linear(morphable_model, affine_from_orthos, image_points_w_contour, vertex_indices_w_contour, mean_plus_blendshapess, lambda, num_shape_coefficients_to_fit);

		// Estimate the blendshape coefficients with the current PCA model estimate:
		current_pca_shape = morphable_model.get_shape_model().draw_sample(pca_shape_coefficients);

		for (int j = 0; j < num_images; ++j) {
			blendshape_coefficientss[j] = fitting::fit_blendshapes_to_landmarks_nnls(blendshapes, current_pca_shape, affine_from_orthos[j], image_points_w_contour[j], vertex_indices_w_contour[j]);

			current_combined_shapes[j] = current_pca_shape + blendshapes_as_basis * Mat(blendshape_coefficientss[j]);
			current_meshs[j] = morphablemodel::sample_to_mesh(current_combined_shapes[j], morphable_model.get_color_model().get_mean(), morphable_model.get_shape_model().get_triangle_list(), morphable_model.get_color_model().get_triangle_list(), morphable_model.get_texture_coordinates());
		}
	}

	//fitted_image_points = image_points;
	return { current_meshs, rendering_paramss }; // I think we could also work with a Mat face_instance in this function instead of a Mesh, but it would convolute the code more (i.e. more complicated to access vertices).
};
 ...
 * @return The fitted model shape instance and the final pose.
 */
inline std::pair<std::vector<core::Mesh>, std::vector<fitting::RenderingParameters>> fit_shape_and_pose(const morphablemodel::MorphableModel& morphable_model, const std::vector<morphablemodel::Blendshape>& blendshapes, const std::vector<core::LandmarkCollection<cv::Vec2f>>& landmarkss, const core::LandmarkMapper& landmark_mapper, std::vector<int> image_widths, std::vector<int> image_heights, const morphablemodel::EdgeTopology& edge_topology, const fitting::ContourLandmarks& contour_landmarks, const fitting::ModelContour& model_contour, int num_iterations = 5, boost::optional<int> num_shape_coefficients_to_fit = boost::none, float lambda = 30.0f)
{
	std::vector<float> pca_coeffs;
	std::vector<std::vector<float>> blendshape_coeffss;
	std::vector<cv::Vec2f> fitted_image_points;
	return fit_shape_and_pose(morphable_model, blendshapes, landmarkss, landmark_mapper, image_widths, image_heights, edge_topology, contour_landmarks, model_contour, num_iterations, num_shape_coefficients_to_fit, lambda, boost::none, pca_coeffs, blendshape_coeffss, fitted_image_points);
};
	} /* namespace fitting */
} /* namespace eos */
